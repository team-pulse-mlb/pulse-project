...
game processor(급상승)·poller(경기 시작)가 판정해 발행한 이벤트의 원본. 사용자별 수신함과 분리해 발화 이력을 남긴다.
...
> 시딩 원천은 poller 파이프라인(추적 경기의 스텁·보강)과 관심 선수 등록([FAVORITE_PLAYERS.md](../policy/FAVORITE_PLAYERS.md))이다. 후자로 추적 경기에 없던 선수도 행으로 존재할 수 있다. 관심 선수 등록 행은 `full_name`이 채워져 있어 `full_name IS NULL` 대상 보강 배치의 갱신 범위 밖이다.

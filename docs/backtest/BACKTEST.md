...
| 지표 | 방법 | 목적 |
|---|---|---|
| 예측력 | 시점 t의 점수와 "이후 12 plays 내 **긴장 이벤트**(리드 변경, 또는 득점 후 점수 차 2 이내)"의 판별력(AUC). 블로우아웃 추가 득점은 양성에서 제외한다 | 점수가 긴장을 결과보다 먼저 잡는지 |
| 알림 빈도 | 알림 임계(`scoring.yml` `alert-score`) 기준 하루 알림 발생 수 분포 | 알림이 하루 2~5건 범위가 되도록 임계·상수 조정 |

예측력 AUC는 두 갈래로 산출한다.

- **AUC(상태 신호)**: 상태 신호(`late_or_extra`·`score_gap`·`pressure`·`count_pressure`·`early_slugfest`) 합만으로 계산한 예측력. 튜닝의 1차 기준이다.
- **AUC(전체 신호)**: `base_score` 전체로 계산한 값. 사후 반응 신호(`recent_score`·`lead_change`·`big_inning`)는 이벤트 직후 점수를 올리는 표시용 신호라 예측력 평가에 불리하게 작용하므로, 이 값은 참고 지표로만 읽는다.

## 3. 튜닝 절차

`scoring.yml` 상수를 그리드 서치로 조정한다. 지표 우선순위는 예측력 > 알림 빈도로 한다. 상수 변경 시 `version`을 올리고 `tune:` PR에 영향 리포트를 첨부한다. 지표는 정답이 아니라 변경 영향을 확인하는 가드레일로 읽는다(예: AUC 하락, 알림 수 급증).
...
| 예측력 | AUC(전체 신호·상태 신호)의 기준↔후보 델타 |
| 경보 플래그 | AUC 하락, 알림 수 급증 등 임계 초과 항목을 강조 표시 |

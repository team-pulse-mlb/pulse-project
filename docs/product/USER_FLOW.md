...
flowchart TD
    ...
    Detail -->|"보호/공개 세그먼트 토글\n(확인창 없이 즉시)"| DetailRevealed["경기 상세 · 공개 모드"]
    DetailRevealed -->|"보호 세그먼트 선택"| Detail
...
| 종료 | 다시보기 구간 수 + AI 헤드라인(보호 모드용, 카드는 항상 보호 모드) | `다시보기 2개 구간 · "경기 후반까지 긴장감이 이어진 경기입니다."` |
...
│ SD  @  CHC                    │ BOS  @  NYY    │
│ [접전 흐름] [득점권 압박]        │ [순위 경쟁]     │
│ (오늘 가장 볼 만한 경기 · 히어로) │ Yankee Stadium │
│                                │ 선발 미확정     │
│                                ├───────────────┤
│                                │ [종료]         │
│                                │ SF  @  LAD     │
...
예정 · 23:05    BOS @  NYY     [순위 경쟁]  Yankee Stadium · 선발 미확정
...
예정 경기는 보호/공개 토글을 노출하지 않는다(공개할 결과 자체가 아직 없음). 데스크톱에서는 §3.4의 추천 사이드바가 본문 오른쪽에 붙는다.
...
상단의 `[보호 | 공개]` 세그먼트 토글에서 공개를 선택하면 다이얼로그 없이 그 자리에서 화면이 즉시 전환된다. 현재 모드는 하이라이트한다.
...
│ 아웃 2 · 볼 3 · 스트라이크 2 · 득점권 주자 있음 │
│ 현재 승부: 타자 Manny Machado · 투수 Shota Imanaga │
│ [접전 흐름] [득점권 압박]                     │
├───────────────────────────────────────────┤
│ 최근 플레이                                  │
│  · 8회 초  스트라이크아웃 (2아웃)             │
...
공개 모드는 보호 모드 항목 전체에 점수, 이닝 초/말, 현재 타자/투수, play 타임라인(원문), 타석 결과, 이닝별 점수, 공개 전용 이벤트를 더해서 보여준다. 보호/공개 전환은 세그먼트 토글 선택 즉시 반영된다. 공개 상태는 이 브라우저(로컬)에만 저장되므로 새로고침해도 유지되지만 다른 기기에는 적용되지 않는다.
...
│ ← 뒤로                         [보호 | 공개] │
│ SF  @  LAD                       [종료]     │
│ "경기 후반까지 긴장감이 이어진 경기입니다."     │  ← AI 헤드라인, 저장된 경우만 표시
├───────────────────────────────────────────┤
│ 긴장 곡선                                    │
...
긴장 곡선은 종료 경기에서만 노출한다. 보호 모드는 이닝 단위 1~5 단계만 표시하고, 원 점수·축 눈금 숫자·하프이닝 방향은 표시하지 않는다. 구간 음영 밴드를 탭하면 해당 이닝 범위의 구간 상세를 열고, 이벤트 마커를 탭하면 보호 표기 라벨만 보여준다. **AI 헤드라인은 모드마다 별도로 생성한 문구다.** 보호 모드 헤드라인은 결과를 드러내지 않는 안전 문구이고(위 예시), 공개 모드 헤드라인은 최종 점수·승패를 반영한 문구다(§4.6 예시 참고). 헤드라인이 없으면 헤드라인 영역 자체를 렌더링하지 않고, 긴장 곡선 이하 영역의 높이와 레이아웃은 그대로 유지한다. **진행 중·예정 경기 화면에는 AI 문구 영역 자체가 존재하지 않는다.**

### 4.6 경기 상세 · 종료 · 공개 모드

상단의 `[보호 | 공개]` 세그먼트 토글에서 공개를 선택하면 즉시 전환된다.
...
| 규칙 | 내용 |
|---|---|
| 공개 전환 | 상세 화면 상단에 `[보호 | 공개]` 세그먼트 토글을 상시 표시한다. 현재 모드를 하이라이트하고, 다른 모드를 선택하면 확인창(alert/confirm/dialog) 없이 즉시 전환한다. 예정 경기는 토글을 노출하지 않는다. |

...
- 다시보기 구간은 곡선 위 음영 밴드로, 이벤트는 곡선 위 노드로 얹는다. 밴드를 탭하면 하단 구간 목록의 해당 구간이 하이라이트되고, 노드를 탭하면 보호 표기 라벨이 툴팁으로 뜬다.
...
- 구간별 문구(AI 요약)는 없다. 구간은 이닝 범위로 설명한다.
...
| 곡선 데이터 없음(점수 이력 미확보 경기) | 곡선 영역 미렌더링, 구간 목록·이벤트 타임라인만 표시 |

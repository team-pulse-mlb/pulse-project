...
이 문서는 배포된 AWS 운영 환경만 다룬다. 로컬 PostgreSQL·Redis 실행은 [`infra/README.md`](../../infra/README.md), S3 원본 수집기와 백필 도구는 [`raw-archive/README.md`](../../raw-archive/README.md)를 따른다.
...
- 로컬 개발자는 `infra/docker-compose.yml`만 사용한다.
- 원본 수집 담당자는 S3·Lambda·EventBridge·CloudWatch의 수집 상태를 확인한다.
- 운영 배포, 보안그룹, 백업·복구, 운영 시크릿 변경은 운영·관측 담당자 영역이다.

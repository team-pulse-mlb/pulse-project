...
- `com.pulse.ai` AiCopyReader, 라이브 Redis·종료 PG 저장, scorer 비동기 트리거
...
| AI 문구 | 창현이 ai-service 문구 생성·검수와 `com.pulse.ai`의 저장 문구 조회·null 반환 계약을 담당하고, 각 화면 담당자가 표시 위치에 맞게 조건부 렌더링한다. |
...
| 창현 | AI 헤드라인 생성·검수 파이프라인, 문구 표시 연동 |

...
PULSE는 3주 안에 통합 데모와 배포를 완성하되, 실제 라이브 경기가 도는 운영 상황을 가정한다. 요구사항에서 정당화되지 않는 기술은 넣지 않고, 같은 문제를 푸는 선택지 중 가장 가벼운 것을 고른다. 각 결정의 상세 논의(대안 비교·검토 과정)는 노션 회의록의 주제별 페이지에 있다.
...
| 인증 | Spring Security + JWT (액세스+리프레시) | Stateless 인증. 리프레시 토큰은 폐기·회전 상태를 가진 보안 데이터라 DB 행으로 관리 |
| DB 접근 | Spring Data JPA + Flyway | 로컬은 `ddl-auto: update`, 배포 환경은 Flyway 마이그레이션만 사용. 베이스라인 V1은 DB 이전에 앞서 확정한다([DB_MIGRATION.md](DB_MIGRATION.md) 참고) |
...
| 배포 | AWS EC2 t3.large 1대 + Docker Compose, RDS, Vercel(프론트) | 컨테이너 8개 단일 호스트. 분리 기준은 장애·부하 성격 |
...
| **재생성 불가 데이터 = PostgreSQL(RDS), 재계산 가능 데이터 = Redis** | 점수 이력·종료 경기 AI 문구는 PG 영속, 라이브 랭킹·라이브 문구는 Redis |
| **판정은 데이터 옆, 전달은 사용자 옆** | 알림 판정은 scorer/poller, fan-out·SSE는 api |

## 3. 배포 토폴로지

EC2 t3.large(8GB) 1대의 Docker Compose에 컨테이너 8개를 올리고, PostgreSQL만 RDS로 분리한다.
...
| Grafana 알림·Loki·OpenTelemetry | 대시보드 1장 범위를 넘는 유지보수 부담 |

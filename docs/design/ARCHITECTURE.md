...
이 그림은 운영 배포 기준이다. `infra/local/docker-compose.yml`은 로컬 개발용으로 PostgreSQL·Redis·RabbitMQ를 띄우고, 운영용 Compose 정의는 [`infra/docker-compose.prod.yml`](../../infra/docker-compose.prod.yml)이다. 운영 리소스 구성과 배포 절차는 [`infra/prod/README.md`](../../infra/prod/README.md)를 따른다. `ai-service`는 담당자 Dockerfile·기동 방식 확정 후 운영 Compose에 편입한다.
...
| ⑫ | 문구 생성 | `ai-service`가 OpenAI API를 호출해 문구 생성 또는 play 번역을 수행한다. 보호 문구에는 스포일러 검수를 적용하고, 번역에는 원문 사실 보존 검수를 적용한다. OpenAI 호출 timeout은 6초다. |

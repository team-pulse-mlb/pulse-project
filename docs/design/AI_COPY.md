...
종료 경기 AI 헤드라인과 다시보기 구간 요약의 생성 트리거, 컨텍스트·HTTP 계약, 검수, 저장, 폴백을 정의한다. AI는 추천 여부를 판단하지 않고, 서버가 전달한 스포일러 세이프 context로 문구만 만든다.

## 1. purpose와 기본 문구

AI 문구는 서버가 만든 안전한 context만 입력으로 사용한다. 생성 결과가 금지 표현이나 점수·승패·우세 정보를 포함하면 기본 문구로 대체한다.

헤드라인·요약은 종료 경기 산출물에만 적용한다. 진행 중·예정 경기 카드와 진행 중 상세 화면은 추천 태그와 보호 모드 정보만 사용한다. 종료 경기 AI 헤드라인과 다시보기 구간 AI 요약은 스포일러 검수를 통과한 안전 문구이므로 보호 모드와 공개 모드(스포일러 공개 모드) 모두에서 같은 문구를 노출할 수 있다. 공개 모드라고 해서 점수·승패·결과를 포함한 별도 AI 문구를 만들지는 않는다.
...
`FINAL_HEADLINE`은 종료 경기 카드와 종료 경기 상세의 보호 모드·공개 모드 응답에 공통으로 사용할 수 있다. `REPLAY_SUMMARY`도 다시보기 구간 목록에서 보호 모드·공개 모드 모두에 내려줄 수 있다. 두 문구는 이미 스포일러 안전 문구로 검수된 값이므로 공개 모드 전용 문구를 별도로 분기하지 않는다.

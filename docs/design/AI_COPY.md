...
- scorer가 유의미한 변화(태그 세트 변화, 추천 상태 진입, 구간 확정, 경기 종료 정리)를 감지한 시점에 비동기로 요청하고, Spring Boot 기준 데드라인(8초) 내 생성+검수 통과 시 저장한다. 실패·검수 불통과·`contextHash` 불일치 시 저장하지 않고 기존 문구를 유지하며 다음 변화 때 재시도한다.

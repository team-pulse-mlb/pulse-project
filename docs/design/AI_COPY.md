...
- scorer가 보호 안전 `game_events`를 영속한 직후 `EVENT_COPY`를 `mode=PROTECTED`로 요청한다. 공개 이벤트 문구는 생성하지 않는다. 저장 조건과 타임아웃은 `FINAL_HEADLINE`과 동일하다.
...
이벤트 문구는 `AiCopyReader`를 거치지 않는다. `GET /api/games/{id}/events?mode=PROTECTED`가 `game_events.copy_protected`를 직접 조회해 `copy`로 반환한다. `copy=null`이면 프론트는 같은 이벤트의 `label`을 폴백으로 표시한다. 공개 모드는 빈 이벤트 목록을 반환한다.

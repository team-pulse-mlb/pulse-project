...
프롬프트의 조립 순서, 목적·모드별 지시, Structured Output과 근거 ID 계약은 [AI_PROMPT_DESIGN.md](AI_PROMPT_DESIGN.md)를 따른다.
...
HTTP 요청 경로에서 LLM을 호출하지 않는다. 문구 생성은 사용자 요청 경로가 아니라 데이터 갱신 시점에 비동기로 수행한다. Spring Boot는 ai-service 응답을 최대 30초까지 기다리며, 이 제한은 사용자 화면 요청의 대기 시간이 아니라 백그라운드 문구 생성 요청을 포기하는 상한이다. ai-service 내부 OpenAI 호출은 목적별 timeout과 최대 시도 횟수를 별도로 적용한다.

- scorer가 경기 종료 정리를 수행할 때 `FINAL_HEADLINE`을 `mode=PROTECTED`·`mode=REVEALED` 두 번 요청한다. ai-service는 FINAL_HEADLINE마다 OpenAI 호출을 최대 20초 동안 1회 시도하고, Spring Boot는 ai-service 응답을 최대 30초까지 기다린다. 생성·Structured Output 파싱·Evidence Guard·Spoiler Guard·최신 `contextHash` 검증을 모두 통과한 결과만 저장한다. 두 모드 요청은 서로 독립적이라 한쪽이 실패해도 다른 쪽 저장에는 영향을 주지 않는다. 헤드라인 요청은 마지막 play와 공개 이벤트 영속이 모두 끝난 뒤에 보낸다. 누락된 종료 헤드라인은 `pulse-headline-backfill` 또는 AI 문구 재처리 배치로 복구할 수 있다.
- `EVENT_COPY`는 이벤트 영속 직후가 아니라 **추천 점수가 급변한 순간(타임라인 하이라이트)**에 `mode=PROTECTED`로 요청한다. 이벤트 태그마다 문구를 만들면 같은 이닝의 유사 이벤트가 저정보·중복 문구로 쌓이므로, 급변 구간의 anchor 이벤트 하나에만 문구를 붙인다. 공개 이벤트 문구는 생성하지 않는다. EVENT_COPY는 실시간 반영을 고려해 OpenAI 호출을 3초씩 최대 2회 시도하며, `spoilerSafe=true`, `fallbackUsed=false`, 최신 `contextHash` 일치 조건을 통과한 경우에만 저장한다.
...
- OpenAI 서비스 내부 재시도와 Spring Boot의 영속 재처리 횟수는 별개다. FINAL_HEADLINE은 긴 단일 호출을 1회만 수행하고, 누락 건은 종료 헤드라인 백필 또는 AI 문구 재처리 배치에서 다시 요청한다. EVENT_COPY는 한 요청 안에서 3초 호출을 최대 2회 시도하며, 재시도 스케줄러가 일정 지연 후 영속 시도 상한(기본 3회)까지 재요청한다. PLAY_TRANSLATION도 한 요청 안에서 3초 호출을 최대 2회 시도하고, 영속 시도 상한(기본 3회)을 넘으면 원문 폴백을 유지한다. 중요 플레이 번역이 저장되면 공개 헤드라인은 경기당 1회만 자동 재생성을 시도하며, `games.final_headline_revealed_regeneration_attempted_at` 원자적 claim으로 재시작·중복 task에서도 멱등성을 보장한다. 상한 초과 시 `null` 또는 원문 폴백 상태가 유지된다.
...
- 타임아웃: Spring Boot → ai-service의 connect timeout은 2초, read timeout은 30초다. ai-service → OpenAI는 FINAL_HEADLINE 20초·1회, EVENT_COPY 3초·최대 2회, PLAY_TRANSLATION 3초·최대 2회로 분리한다. 공통 OpenAI timeout 6초는 목적별 설정이 없거나 유효하지 않을 때만 사용하는 fallback이다. OpenAI 응답 이후 Structured Output 파싱과 검수 결과를 반환할 시간을 확보하도록 Spring Boot read timeout을 가장 길게 둔다.
- ai-service는 무상태다. 캐시·DB에 직접 쓰지 않고 생성·검수 결과를 응답으로만 반환한다.
- ai-service는 대체 문구를 생성하거나 반환하지 않는다. 현재 계약에서 `fallbackUsed`는 항상 `false`다.
- 저장 조건: FINAL_HEADLINE은 `spoilerSafe=true`, `fallbackUsed=false`, 최신 `contextHash` 일치와 Evidence Guard 통과를 모두 만족한 응답만 저장한다. EVENT_COPY는 `spoilerSafe=true`, `fallbackUsed=false`, 최신 `contextHash` 일치인 응답만 저장한다. PLAY_TRANSLATION은 `translatedText`가 비어 있지 않고 번역 Guard를 통과하며 `fallbackUsed=false`, 최신 `contextHash` 일치일 때만 저장한다.
...
### 5.3 FINAL_HEADLINE 근거 계약

FINAL_HEADLINE은 Structured Output으로 `safeTitle`, `usedFactIds`, `usedPlayIds`를 반환한다. `usedFactIds`와 `usedPlayIds`에는 모델이 실제 문구 생성에 사용한 근거 ID만 넣는다.

- `mode=PROTECTED`에서는 결과 사실이나 play 원문을 입력하지 않으므로 두 evidence 배열은 빈 배열이어야 한다.
- `mode=REVEALED`에서는 `summaryFacts`와 `verifiedPlays`에 실제 존재하는 ID만 사용할 수 있다.
- 존재하지 않는 ID, 값이 `null`인 fact, 중복 ID, PROTECTED 요청의 결과 evidence 사용은 반려한다.
- 선수명은 `usedPlayIds`가 가리키는 play의 batter 또는 pitcher와 연결돼야 한다.
- `역전`, `끝내기`, `영봉`, `연장`, 총득점처럼 서버가 계산한 사실은 대응하는 `summaryFacts` 근거가 있어야 한다.
- `결승타`는 동일 play의 `DECISIVE_SCORE`와 `HIT`, `동점타`는 `TYING_SCORE`와 `HIT`, `쐐기타`는 `INSURANCE_SCORE`와 `HIT`가 함께 확인돼야 한다.
- `리드`, `우세`, `열세`, `따라붙다`, `실점` 등 경기 흐름 표현도 대응하는 play fact tag가 있는 경우에만 허용한다.
- `총 N득점`, `양 팀 합계 N득점`의 숫자는 `summaryFacts.totalRuns`와 정확히 일치해야 한다.

Evidence Guard와 Spoiler Guard 중 하나라도 실패하면 `safeTitle`을 저장하지 않는다. 검수는 AI 문구가 입력 context 범위 밖의 사실을 추가하거나, 올바른 사실을 잘못된 팀·선수·점수에 연결하는 것을 차단한다.

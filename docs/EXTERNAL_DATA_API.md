...
> **실측 기준일: 2026-07-02~03 (시즌 중).** GOAT 플랜 키로 전체 엔드포인트를 실제 호출해 확인한 결과를 "실측"으로 표기했다. 진행 중 경기의 반영 지연은 7/2 라이브 슬레이트 아카이브로 측정 완료 — play는 폴링 주기(~20초)마다 연속 유입된다(7장 참고). 타순 공개 시점과 429 `Retry-After`는 2026-07-04 추가 실측(7장).
...
- 한도 초과 시 `429 Too Many Requests` + 평문 body(`"Too many requests, please try again later."`)를 반환하며, **응답에 `Retry-After` 헤더(정수 초, 다음 리셋까지 남은 시간 = `x-ratelimit-reset`과 일치)가 포함된다**(2026-07-04 실측). 폴러 백오프는 이 값을 그대로 신뢰해 대기하면 된다. 리셋은 1분 단위 창(window)으로, 초과분만 429가 되고 창이 넘어가면 예산이 리필된다.
...
| 데이터 | 등장·갱신 시점 (실측) |
|---|---|
| 경기 일정 | 최소 7일 뒤 날짜까지 제공 확인 |
| 선발 예상 투수 (`/lineups`) | 경기 36시간 전에도 제공 (내일 저녁 경기 6개 중 5개는 양팀, 1개는 한 팀만 확정) |
| 타순 (`batting_order`) | 경기 5.7시간 전에도 미공개(당일 9경기 전부). 종료 경기에는 팀당 타순 9명+선발 투수로 총 20행 제공. **공개 실측(2026-07-04, 07-02~04 아카이브 12경기): 경기 시작 T-2.2h~T-4.4h, 중앙값 ≈ T-3.2h, 평균 ≈ T-3.3h**(7장) |
...
- **타순 공개 실측(2026-07-04)**: 경기 시작 T-2.2h~T-4.4h(중앙값 ≈ T-3.2h) 사이에 공개(7장). 폴러는 라인업 폴링을 **T-5h부터 촘촘히** 돌리면 공개 순간을 놓치지 않는다.
...
- 주의: `avg`, `obp`, `slg`, `era`는 **시즌 누적 기준이다**(2026-07-03 재확인 완료: 당일 2타수 0안타 선수의 `avg`가 0.215, 4타수 1안타가 0.206 — 경기 성적과 무관). 경기 단일 성적은 카운트 필드로 직접 계산할 것.
- 진행 중 경기의 실시간 제공 여부는 미검증(7장).
...
| `fielding_fip` | 이름과 달리 FIP 아님 | `65.7`~`97`(에이스), `3.67`(부진 투수) | **사용 안 함 확정** (2026-07-03 실측: FIP 스케일 아님, 성적과 무상관) |
...
- [x] **타순 공개 시점 정밀 실측** — 완료(2026-07-04). S3 `raw/lineups` 아카이브(2026-07-02~04, 스냅샷 66건)를 `scripts/analyze-lineup-reveal.py`로 집계. 수집 창 안에서 `batting_order`가 null→공개로 전환된 게 포착된 12경기 기준 **공개는 경기 시작 T-2.2h ~ T-4.4h(중앙값 ≈ T-3.2h, 평균 ≈ T-3.3h)** — 리그 관행("경기 1~4시간 전")을 실측으로 확정, T-3h 부근 집중. (수집 창이 열리기 전 이미 공개돼 있던 경기는 좌측 절단으로 하한만 확인, 모두 이와 정합.) → **폴러는 라인업 폴링을 T-5h부터 촘촘히** 돌리면 공개 순간을 놓치지 않는다. 더 정밀한 분포가 필요하면 15분 등간격 수집을 며칠 더 쌓아 같은 스크립트로 재집계.
- [ ] `/stats`가 진행 중 경기에 실시간 제공되는지 확인 (제공된다면 라이브 박스스코어로 활용 가능). → **미검증**: 2026-07-04 확인 시도 시 진행 중 경기가 없었다(라이브 슬레이트 미시작). **확인 방법**: 라이브 슬레이트 시간대(KST 오전)에 진행 중 `game_id`로 `/stats?game_ids[]=`를 수동 1콜. 행이 존재하고 30분 뒤 재호출에서 카운트 필드가 증가해 있으면 실시간 제공 확정.
- [x] **429 응답의 `Retry-After` 헤더** — 완료(2026-07-04). `/teams`로 한 리셋 창(60초) 안에 600콜 초과를 유발(633건 200 / 867건 429). **429 `Too Many Requests`에 `Retry-After` 헤더가 붙는다** — 정수 초(관측값 5·9초 = 다음 리셋까지 남은 시간, `x-ratelimit-reset`과 일치), `x-ratelimit-remaining: 0`, body는 평문 `"Too many requests, please try again later."`. → 폴러 백오프는 `Retry-After` 값을 그대로 신뢰해 대기하면 된다(2장에도 반영).

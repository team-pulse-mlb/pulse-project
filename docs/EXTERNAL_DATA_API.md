...
> **실측 기준일: 2026-07-02~03 (시즌 중).** GOAT 플랜 키로 전체 엔드포인트를 실제 호출해 확인한 결과를 "실측"으로 표기했다. 진행 중 경기의 반영 지연은 7/2 라이브 슬레이트 아카이브로 측정 완료 — play는 폴링 주기(~20초)마다 연속 유입된다(7장 참고).
...
| 타순 (`batting_order`) | 경기 5.7시간 전에도 미공개(당일 9경기 전부). 종료 경기에는 팀당 타순 9명+선발 투수로 총 20행 제공. 공개는 통상 경기 1~4시간 전(리그 관행) — 정확한 시점은 추가 실측 필요(7장) |
...
- 타순 공개는 통상 경기 1~4시간 전(리그 관행). 정확한 공개 시점은 추가 실측 필요(7장 확인 항목).
...
- [ ] **타순 공개 시점 정밀 실측**: T-6h ~ T-0 구간을 15분 간격으로 모니터링해 실제 공개 시각 분포 확인. → **확인 방법**: 추가 수집 불필요 — 개발 수집기가 이미 1분 주기로 `lineups` 원본을 적재 중이므로(데이터 처리 흐름 6.3장), 며칠 치가 쌓이면 경기별로 `batting_order`가 처음 non-null이 된 객체의 `observed_at`과 경기 시작 시각의 차이를 집계하는 분석 스크립트만 돌리면 된다.
- [ ] `/stats`가 진행 중 경기에 실시간 제공되는지 확인 (제공된다면 라이브 박스스코어로 활용 가능). → **확인 방법**: 라이브 슬레이트 시간대(KST 오전)에 진행 중 `game_id`로 `/stats?game_ids[]=`를 수동 1콜. 행이 존재하고 30분 뒤 재호출에서 카운트 필드가 증가해 있으면 실시간 제공 확정.
- [ ] 429 응답에 `Retry-After` 헤더가 오는지 확인 (의도적으로 유발하지 않아 미확인). → **확인 방법**: (권장) 폴러의 429 처리 경로에 응답 헤더 전체 로깅을 심어 두고 자연 발생 시 확인. 급하면 새벽 유휴 시간에 `/teams` 같은 가벼운 엔드포인트로 1분간 600콜을 잠깐 초과시켜 헤더를 직접 본다(`x-ratelimit-reset` 기준 1분 내 리셋).

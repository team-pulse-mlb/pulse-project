...
PA만 예외인 이유: PA는 운영 DB에 영속하지 않는 유일한 원본이다([DB_SCHEMA.md](DB_SCHEMA.md) §E-1). 다른 데이터는 DB가 자체 보존 수단이지만, PA는 이 아카이브가 없으면 `game_events` 추출 룰 변경 시 소급 재추출과 PA 신호 백테스트 확장이 외부 API의 과거 데이터 보존 정책에 의존하게 된다. 업로드 실패는 운영 경로(적재·ScoreTask)에 전파하지 않는다.

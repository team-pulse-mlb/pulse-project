...
backend 세 역할은 같은 Java 이미지를 사용하고, `pulse-ai-service`는 별도 Python 이미지를 사용한다. Flyway는 `pulse-api`에서만 실행한다. 경기 처리기 활성화 설정 `PULSE_SCORER_ENABLED`와 Prometheus의 `role="scorer"`는 기존 운영 계약을 유지한다.
...
- `up{job="pulse-backend"}`의 `role="api"`·`role="poller"`·`role="scorer"`가 모두 `1`인지 확인

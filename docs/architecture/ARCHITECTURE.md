...
| ⑪ | AI 문구 요청 | `pulse-scorer`가 종료 헤드라인, 보호 이벤트 문구, 공개 최근 플레이 번역을 `ai-service`에 비동기로 요청한다. Spring Boot의 ai-service 호출 timeout은 30초다. |
| ⑫ | 문구 생성 | `ai-service`가 OpenAI API를 호출해 문구 생성 또는 play 번역을 수행한다. 보호 문구에는 스포일러 검수를 적용하고, 번역에는 원문 사실 보존 검수를 적용한다. OpenAI 호출 timeout은 Spring Boot 호출 timeout(30초)보다 짧게 둔다(FINAL_HEADLINE은 최대 20초, 그 외 문구 생성·번역은 더 짧게). |
...
**장애 격리의 범위 제한**: 위 격리는 **프로세스 단위**에 한정된다. 현재 운영은 단일 EC2·단일 Redis·단일 RabbitMQ 구성이라, poller·scorer·api가 서로의 오류를 격리해도 호스트·Redis·RabbitMQ 자체가 죽으면 전체가 영향을 받는다. 이는 결함이 아니라 비용·일정 제약에 따른 의도적 범위이며, 고가용 이중화는 현 단계 목표가 아니다.

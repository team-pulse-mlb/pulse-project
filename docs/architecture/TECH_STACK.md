...
| AI 서비스 | Python 3.12 + FastAPI + Pydantic | Spring이 조립한 safeContext를 요청 DTO로 검증하고, 추천 판단·저장 책임 없이 문구 생성·번역·검수만 수행 |
| AI 생성 계약 | OpenAI Responses API + Structured Outputs(JSON Schema) | FINAL_HEADLINE·EVENT_COPY·PLAY_TRANSLATION별 출력 스키마를 강제하고 형식 오류와 임의 필드를 서비스 경계에서 거절 |
| AI 후처리 검증 | Evidence Guard + Spoiler Guard + Translation Guard + PyYAML 용어집 | 모델 출력의 사용 근거·모드별 스포일러·원문 사실·야구 용어 보존을 결정론적으로 검증 |
...
| **판정은 데이터 옆, 전달은 사용자 옆** | 알림 판정은 game processor/poller, fan-out·SSE는 api |
...
| 컨테이너 | 내용 |
|---|---|
| `pulse-api` | REST·SSE·스포일러 보호 DTO·알림 전달 (Spring) |
| `pulse-poller` | 상태별 폴링·원본 저장·계산 요청 발행 (Spring) |
| `pulse-game-processor` | 점수 계산·경기 이벤트 처리·알림 판정·AI 트리거 (Spring) |
| `ai-service` | Responses API 기반 FINAL_HEADLINE·EVENT_COPY·PLAY_TRANSLATION 생성과 Structured Output·Evidence·Spoiler·Translation Guard 검수 (FastAPI) |

...
class AiEventCopyGenerator {
    ...
    GenerationStatus generateSynchronously(long gameId, long eventId, AiCopyMode mode) {
        ...
        if (response.isEmpty()) {
            log.warn("AI 이벤트 문구 응답 없음/호출 실패: gameId={} eventId={} mode={}",
                    gameId, eventId, mode);
    ...
    private GenerationStatus processResponse(
        ...
        if (!hasLatestContextHash(gameId, eventId, mode, response)) {
            log.info("AI 이벤트 문구 stale 응답 폐기: gameId={} eventId={} mode={} responseContextHash={} latestContextHash={}",
                    ...
                    latestContextHash(gameId, eventId, mode));
    ...
    private String latestContextHash(long gameId, long eventId, AiCopyMode mode) {
        return contextReader.eventCopyContext(gameId, eventId, mode)
                .map(EventCopyContext::contextHash)
                .orElse(null);
    }
